...
void sendDiag(String apiTokenL)
{
  ...
  jsonContent["settings"]["firmware"]["ver"] = "4";
...
void setup()
{
  ...
  //Wifi Conection
    ...
    Serial.println("Heap: " + ESP.getFreeHeap());
  #endif
...
void loop()
{
  ...
  if (!waitForWifi(1))
  {
    ...
    #ifdef ENABLE_SERIAL_PRINT
      Serial.println(String(configPage) + "Performing Reconect in" + String((WIFI_RECONNECT_INTERVAL - ((long)millis() - last_micros_wifi))));
    #endif
